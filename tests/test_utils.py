...
```'''
        ...
        mock_api_src.getPostDate.return_value = formatdate(timeval=datetime.datetime.now().timestamp(), localtime=True)
...
more text"""
    ...
    @patch("manage_agenda.utils.MistralClient")
    @patch("manage_agenda.utils.GeminiClient")
    def test_select_llm_non_interactive_always_gemini(self, mock_gemini_client, mock_mistral_client, mock_ollama_client):
